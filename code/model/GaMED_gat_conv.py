...
class GATConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
                soft_edge_index=None, edge_weight=None, soft_edge_weight=None,
        ...
        if soft_edge_index is not None:
            edge_index = torch.cat([edge_index, soft_edge_index], dim=1)
            edge_weight = torch.cat([edge_weight, torch.clamp(soft_edge_weight, 0, 1)], dim=0)
    ...
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        if edge_weight is not None:
            alpha = edge_weight.unsqueeze(-1) * alpha
